...
async function sendVerificationEmail({ to, name, token }) {
    ...
    await getTransport().sendMail({
        from,
        to,
        subject: 'Complete your SPIDAC - Digital Tech Assurance registration',
        text: `Hi ${name},\n\nComplete your SPIDAC - Digital Tech Assurance registration by opening this link:\n${verifyUrl}\n\nThis link expires in 24 hours. If you did not create this account, you can ignore this email.`,
        html: `<p>Hi ${escapeHtml(name)},</p><p>Complete your SPIDAC - Digital Tech Assurance registration by clicking the button below.</p><p><a href="${verifyUrl}" style="display:inline-block;padding:11px 16px;background:#6366f1;color:#fff;text-decoration:none;border-radius:6px;font-weight:700">Verify email address</a></p><p>Or open this link:</p><p>${verifyUrl}</p><p>This link expires in 24 hours. If you did not create this account, you can ignore this email.</p>`,
    });
}
...
async function sendMonitorDigest({ to, userName, monitors, baseUrl }) {
    ...
    const subject = 'SPIDAC — Weekly monitor digest';
...
<h2 style="color:#111827">SPIDAC — Weekly Monitor Digest</h2>
...
<p style="color:#9ca3af;font-size:0.8em">You receive this weekly digest because you have monitors configured in SPIDAC. To stop receiving these emails, remove recipients from your monitors.</p>
</div>`;
    const text = `SPIDAC Weekly Monitor Digest\n\n${monitors.map(m => `${m.name} (${m.url}): ${m.last_run_status || 'not run'}`).join('\n')}\n\nView monitors: ${appUrl}/app/monitors`;
...
async function sendMonitorAlert({ monitor, status, criticalCount, highCount, reportUrl }) {
    ...
    const subject = `[SPIDAC Alert] ${escapeHtml(monitor.name)} — threshold exceeded`;
    const lines = [];
    ...
    const text = `SPIDAC Monitor Alert\n\nMonitor: ${monitor.name}\nURL: ${monitor.url}\nStatus: ${status}\nFindings: ${summary}\n\nView report: ${url}`;
    const html = `<div style="font-family:sans-serif;max-width:600px">
<h2 style="color:#dc2626">SPIDAC Monitor Alert</h2>
